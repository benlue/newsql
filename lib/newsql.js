...
exports.find = function(expr, query, cb) {
	var  cmd = {op: 'list'},
		 realQuery = {},
		 filter = retrieveFilter(query, realQuery);
	query = realQuery;
				...
				if (err)
					...
					if (option.conn)  {
						var  conn = option.conn,
							 ...
							 updOption = {expr: updateTemp.value(), conn: conn};

						if (conn.inTransaction())
							groupUpdate(null, updOption, info, pk, rows, cb);
						else
							conn.beginTransaction(function(err) {
					            if (err)
					                cb(err);
					            else
					            	groupUpdate(conn, updOption, info, pk, rows, cb);
					        });
					}
					else
						exports.getConnection(function(err, conn)  {
							if (err)
			            		...
			            		var  updOption = {expr: updateTemp.value(), conn: conn};
...
function  retrieveFilter(input, query)  {
	...
	return  filters.length > 1  ?  {op: 'and', filters: filters} : filters[0];
};
